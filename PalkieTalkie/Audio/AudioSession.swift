    ...
    static func configureForFullDuplexVoice() throws {
        ...
        do {
            try session.setCategory(
                .playAndRecord,
                // `.default` mode (NOT `.videoChat` / `.voiceChat`) — those modes enable iOS's voice processing AGC + noise gate at the session level, which silences sub-`-11 dBFS` audio as "background noise". OpenAI Realtime sends quieter TTS than PersonaPlex Opus, so the gate killed playback entirely. We still need AEC (so the AI doesn't hear itself), but we enable it surgically on the mic input via `inputNode.setVoiceProcessingEnabled(true)` in `AudioStreamer.start()` rather than session-wide.
                mode: .default,
