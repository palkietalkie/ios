...
///
/// Why this exists: NVIDIA's PersonaPlex server (`sphn.OpusStreamWriter`) emits Ogg-encapsulated Opus over the
/// WebSocket. Swift's `Opus.Decoder.decode(_:)` expects RAW Opus packets, not Ogg pages. Feeding Ogg page bytes
/// directly into the raw decoder fails silently (we `try?`), and the user hears nothing while the model is happily
/// generating audio server-side.
///
/// Bytes can arrive in arbitrarily-sized WS messages; an Ogg page can be split across messages and multiple pages can
/// land in a single message. The reader buffers partial input and emits whole packets as they become complete.
///
/// Header pages (OpusHead, OpusTags) are recognized and skipped — those carry metadata for an Ogg-aware decoder, not
/// audio to play.
final class OggOpusReader {
    ...
    /// Try to parse one full Ogg page from the head of `buffer`. Returns nil if there's not enough bytes yet for a
    /// complete page. On success, consumes the page from `buffer`.
    private func nextPage() -> OggPage? {
        ...
        guard let magicIdx = indexOfOggSMagic(in: buffer) else {
            // No magic in buffer; drop everything (it can't form a page). Keep last 3 bytes in case "Ogg" prefix is
            // split across reads.
        ...
        // If the page ends mid-packet (last segment was 255), the partial trailing bytes belong to a packet that
        // continues on the next page. We add a "partial" entry; the next page will start with the continuation. For
        // simplicity we treat full-page-ends-mid-packet as the packet ending here too — fine for streaming
        // silence/voice frames which are well under 255 bytes each.
        if current > 0 {
            packetSizes.append(current)
        }
        ...
        // Skip header packets (OpusHead, OpusTags). Detect by content: OpusHead starts with "OpusHead", OpusTags with
        // "OpusTags". The BOS flag marks the first page but a BOS page can also carry audio in some Ogg variants; check
        // magic explicitly.
    ...
    private func indexOfOggSMagic(in data: Data) -> Int? {
