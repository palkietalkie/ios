...
/// PCM16 variant for the OpenAI Realtime path. Mic side emits raw PCM16 frames; speaker side accepts raw PCM16 chunks.
/// Separate protocol so the OpenAI-specific pump doesn't accidentally end up running over the Opus path (or vice
/// versa).
protocol PCM16AudioStreamerType: AnyObject, Sendable {
    ...
    /// Stop and clear the player's queued buffers so the user hears immediate silence when they barge in over the AI.
    /// Player should be ready to receive new buffers immediately after.
    func interruptPlayback() async
}

extension AudioStreamer: PCM16AudioStreamerType {}

/// Pumps audio in both directions between an `AudioStreamerType` and a `PersonaPlexSessionType`. Separating this from
/// `SessionController` keeps the orchestrator free of audio plumbing — and lets us test the pump's start/stop behaviour
/// without booting AVAudioEngine.
actor AudioPump {
    private var tasks: [Task<Void, Never>] = []

    /// Starts mic→server and server→speaker pumps. Each runs until either side finishes its async stream or `stop()` is
    /// called.
    func start(streamer: AudioStreamerType, session: PersonaPlexSessionType) async {
        await stop()

        let micTask = Task.detached {
            // Block until the server sends its handshake byte. Sending audio earlier breaks sphn's Ogg-Opus stream
            // parsing because the server's recv_loop only starts after step_system_prompts_async (~30s on cold start).
            // See PersonaPlexClient.waitForServerHandshake.
    ...
    /// PCM16-mode variant for any `RealtimeClient` (OpenAI today; future PCM16-speaking providers tomorrow). Mic emits
    /// raw PCM16 frames straight onto the wire; server-side PCM16 chunks go through `playPCM16`.
