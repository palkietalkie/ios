...
/// Production conformer for `Authing`. Wraps the Clerk SDK so the rest of the app doesn't depend on Clerk types directly. All public methods are async and hop to MainActor internally because Clerk's user / session state lives on the main actor.
///
/// Used to be `ClerkAuth` (a `@MainActor` final class with a `static let shared` accessor); singleton was removed in favor of constructor injection via `@Environment(\.authing, …)`. Tests now hand in a `StubAuthing` instead of touching Clerk at all.
