...
/// PushNotifications + AppDelegate. APNs registration can't be triggered in tests; we cover the device-token hex conversion and the foreground-banner UNUserNotificationCenter delegate.
@MainActor
final class PushNotificationsTests: XCTestCase {
    ...
    func testDidRegisterConvertsTokenToHex() {
        let pn = PushNotifications(backend: makeBackend())
        // 4 bytes → 8 hex chars. The function POSTs to backend on a fire-and-forget Task — we don't await it. The conversion path runs synchronously inside didRegister and is the load-bearing logic.
        pn.didRegister(deviceToken: Data([0xDE, 0xAD, 0xBE, 0xEF]))
    }

    // UNNotification has no public initializer — synthesizing one safely in unit tests isn't possible without a real notification delivery. The willPresent delegate method body is two lines; we accept the partial-coverage trade.
